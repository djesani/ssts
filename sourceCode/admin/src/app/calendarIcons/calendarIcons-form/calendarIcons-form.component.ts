...
export class CalendarIconsFormComponent implements OnInit {
  ...
  ngAfterViewInit() {
    setTimeout(() => this.inputEl.nativeElement.focus(), 2000);
 }
  ...
  loadData() {
    ...
    this.calendarIconsService.getAll().subscribe((data: any[]) => {
      ...
      if (this.isEdit) {
        this.event = this.calendarIcons.find(event => event.filename == this.getId);
        this.setData();
        // this.loader.stop();
      } else {
        this.event = {};
  ...
  saveEvent() {
    this.getData();
    if (this.isEdit) {
      this.calendarIconsService.update(this.event.filename, this.event)
        .subscribe(
          data => { console.log('edit-data', data) },
          error => { this.router.navigate(['/calendarIcons']) },
          () => { console.log('success'); this.router.navigate(['/calendarIcons']) }
        );
    } else {
      this.calendarIconsService.add(this.event)
        .subscribe(
