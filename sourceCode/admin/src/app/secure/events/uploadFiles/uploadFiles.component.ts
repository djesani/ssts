...
export class UploadFilesComponent implements OnInit {
  ...
  uploadFileConfigSetDefault() {
    if (
      this.uploadFileConfig.fileTypeAcceptError == undefined ||
      this.uploadFileConfig.fileTypeAcceptError == ""
    ) {
      this.uploadFileConfig.fileTypeAcceptError =
        "We couldn't upload this file as this file format is not allowed";
    }
    if (
      this.uploadFileConfig.fileMaxSizeError == undefined ||
      this.uploadFileConfig.fileMaxSizeError == ""
    ) {
      this.uploadFileConfig.fileMaxSizeError =
        "We couldn't upload this file as it's more than 10MB";
    }
    if (
      this.uploadFileConfig.fileMinSizeError == undefined ||
      this.uploadFileConfig.fileMinSizeError == ""
    ) {
      this.uploadFileConfig.fileMinSizeError =
        "We couldn't upload this file as it's less than 1KB";
    }
    if (
      this.uploadFileConfig.fileNoContentError == undefined ||
      this.uploadFileConfig.fileNoContentError == ""
    ) {
      this.uploadFileConfig.fileNoContentError =
        "We couldn't upload this file as it's 0 bytes";
    }
    if (
      this.uploadFileConfig.filesMaxUploadAllowedError == undefined ||
      this.uploadFileConfig.filesMaxUploadAllowedError == ""
    ) {
      this.uploadFileConfig.filesMaxUploadAllowedError =
        "You've reached the limit of uploadFiles";
    }
    if (
      this.uploadFileConfig.fileTypeAccept == undefined ||
      this.uploadFileConfig.fileTypeAccept == ""
    ) {
      this.uploadFileConfig.fileTypeAccept = ".doc, .docx, .pdf, .jpg, .jpeg";
    }
    if (
      this.uploadFileConfig.fileUploadMultiple == undefined ||
      this.uploadFileConfig.fileUploadMultiple == ""
    ) {
      this.uploadFileConfig.fileUploadMultiple = true;
    }
    if (
      this.uploadFileConfig.filesMaxUploadAllowed == undefined ||
      this.uploadFileConfig.filesMaxUploadAllowed == ""
    ) {
      this.uploadFileConfig.filesMaxUploadAllowed = 5;
    }
    if (
      this.uploadFileConfig.fileMaxSize == undefined ||
      this.uploadFileConfig.fileMaxSize == ""
    ) {
      this.uploadFileConfig.fileMaxSize = 10485760; // 10MB in bytes,
    }
    if (
      this.uploadFileConfig.fileMinSize == undefined ||
      this.uploadFileConfig.fileMinSize == ""
    ) {
      this.uploadFileConfig.fileMinSize = 1024; // bytes
    }
    if (
      this.uploadFileConfig.uploadFileAddError == undefined ||
      this.uploadFileConfig.uploadFileAddError == ""
    ) {
      this.uploadFileConfig.uploadFileAddError = "Error in adding uploadFile";
    }
    if (
      this.uploadFileConfig.uploadFileRemoveError == undefined ||
      this.uploadFileConfig.uploadFileRemoveError == ""
    ) {
      this.uploadFileConfig.uploadFileRemoveError =
        "Error in removing uploadFile";
    }
    if (
      this.uploadFileConfig.uploadFilesRequiredError == undefined ||
      this.uploadFileConfig.uploadFilesRequiredError == ""
    ) {
      this.uploadFileConfig.uploadFilesRequiredError =
        "Please attach at least one file";
    }
    if (
      this.uploadFileConfig.labelFileUpload == undefined ||
      this.uploadFileConfig.labelFileUpload == ""
    ) {
      this.uploadFileConfig.labelFileUpload = "Attach file(s)";
    }
    if (
      this.uploadFileConfig.labelFileUploadIconClass == undefined ||
      this.uploadFileConfig.labelFileUploadIconClass == ""
    ) {
      this.uploadFileConfig.labelFileUploadIconClass = "fa-plus";
    }
    if (
      this.uploadFileConfig.labelRemove == undefined ||
      this.uploadFileConfig.labelRemove == ""
    ) {
  ...
  ngOnInit() {
    ...
    this.formClass = this.uploadFileConfig.uploadFileType
      .replace(/ /g, "-")
      .toLowerCase();
  ...
  uploadFilesFunction(fileEvent: any) {
    ...
    files.forEach((file) => {
      ...
      let uploadFileAcceptArray = this.uploadFileConfig.fileTypeAccept
        .replace(/ /g, "")
        .split(",");
      ...
      if (
        ...
        this.uploadFilesUploadedCount = this.uploadFiles.filter(function (
          element
        ) {
